...
class FolderPilotProvider {
    runner;
    outputChannel;
    _onDidChangeTreeData = new vscode.EventEmitter();
    onDidChangeTreeData = this._onDidChangeTreeData.event;
    skillCache = new Map();
    globalSkillTemplates = new Map();
    constructor(runner, outputChannel) {
        this.runner = runner;
    ...
    async pickSkill() {
        ...
        const selected = await vscode.window.showQuickPick(allSkills.map((skill) => ({
            label: skill.description,
    ...
    async getSkillsForFolder(folderUri) {
        ...
        for (const name of supportedSkillNames) {
            ...
            if (!template) {
                this.outputChannel.appendLine(`FolderPilot: Skill ${name} not found in workspace .agent/interactive_skills for ${folderUri.fsPath}.`);
                continue;
            }
            skills.push({
                ...template,
    ...
    async getGlobalSkillTemplates(folderUri) {
...
class SkillRunner {
    ...
    enqueue(request) {
        ...
        if (total >= 5) {
            if (this.queue.length > 0) {
                ...
                if (dropped) {
                    vscode.window.showWarningMessage(`FolderPilot: Dropped queued skill ${dropped.skill.description} to keep queue under limit.`);
                }
            }
    ...
    async executeSkill(skill, token) {
        await vscode.window.withProgress({
            ...
            const cachedOutputs = await restoreOutputsFromCache(skill.folderUri, skill.name, cacheKey, this.outputChannel);
...
async function buildSkillCacheKey(skill, inputValues) {
...
async function collectInputFileStats(folderUri) {
    const stats = [];
    const visit = async (current, relativeDir) => {
...
async function restoreOutputsFromCache(folderUri, skillName, cacheKey, outputChannel) {
    ...
    if (restoredOutputs.length === 0) {
        outputChannel.appendLine(`FolderPilot: Cache manifest found for ${skillName}, but no artifacts were available for ${cacheKey.slice(0, 12)}.`);
    }
    return restoredOutputs;
}
async function saveOutputsToCache(folderUri, skillName, cacheKey, outputs, outputChannel) {
    const descriptors = [];
...
async function readSkillCacheManifest(folderUri, skillName) {
...
async function writeSkillCacheManifest(folderUri, skillName, manifest) {
    const manifestUri = getSkillCacheManifestUri(folderUri, skillName);
...
function getSkillCacheManifestUri(folderUri, skillName) {
    return vscode.Uri.joinPath(folderUri, '.agent', 'cache', `${sanitizeFileName(skillName)}.manifest.json`);
}
function getCacheArtifactUri(folderUri, skillName, cacheKey, relativeOutputPath) {
    return vscode.Uri.joinPath(folderUri, '.agent', 'cache', 'artifacts', sanitizeFileName(skillName), cacheKey, relativeOutputPath);
}
function toRelativePath(folderUri, fileUri) {
...
async function clearCacheForSkill(skill, outputChannel) {
...
async function findAgentFolders() {
...
async function parseSkillTemplate(fileUri, outputChannel) {
    ...
    let meta;
    try {
        meta = yaml.parse(parsed.frontmatter);
    }
    catch (error) {
    ...
    const outputFormat = typeof meta.output_format === 'string'
        ? meta.output_format
        : typeof meta.outputFormat === 'string'
            ? meta.outputFormat
            : undefined;
    const inputs = typeof meta.inputs === 'object' && meta.inputs ? meta.inputs : {};
...
function getInteractiveSkillsListUri(folderUri) {
    return vscode.Uri.joinPath(folderUri, '.agent', 'interactive_skills.yaml');
}
async function readInteractiveSkillNames(folderUri, outputChannel) {
    ...
    try {
        ...
        outputChannel.appendLine(`FolderPilot: Failed to read interactive_skills.yaml in ${folderUri.fsPath}: ${String(error)}`);
        return [];
    }
}
function parseSkillNameListFromYaml(content) {
    try {
...
function parseSkillNameListFromLines(content) {
    return content
        .split(/\r?\n/)
        .map((line) => line.trim())
...
async function resolveInputs(skill) {
    const resolved = {};
...
async function buildPrompt(skill, inputs, outputRelativePath, outputFormat) {
...
async function listFolderEntries(folderUri) {
    try {
...
async function requestCopilotResponse(prompt, token, outputChannel) {
    ...
    try {
        ...
        for (const model of models) {
            for (let index = 0; index < attemptPrompts.length; index += 1) {
                ...
                try {
                    const response = await model.sendRequest([vscode.LanguageModelChatMessage.User(attemptPrompts[index])], {}, token);
                    const text = await collectResponseText(response);
                    if (text.trim()) {
                        return text;
                    }
                    lastError = new Error('Copilot returned an empty response.');
                    outputChannel.appendLine(`FolderPilot: Empty response from model ${model.id} (attempt ${index + 1}/${attemptPrompts.length}).`);
                }
                catch (error) {
                    lastError = error;
                    outputChannel.appendLine(`FolderPilot: Model ${model.id} failed (attempt ${index + 1}/${attemptPrompts.length}): ${String(error)}`);
        ...
        if (lastError instanceof vscode.LanguageModelError) {
            vscode.window.showErrorMessage(`FolderPilot: Copilot request failed (${lastError.code}). Ensure Copilot is installed and signed in.`);
            return null;
        }
        if (isNoChoicesError(lastError)) {
            vscode.window.showErrorMessage('FolderPilot: Copilot returned no choices for this request. Try again, or simplify the skill prompt.');
            return null;
        }
        ...
        if (error instanceof vscode.LanguageModelError) {
            vscode.window.showErrorMessage(`FolderPilot: Copilot request failed (${error.code}). Ensure Copilot is installed and signed in.`);
            return null;
        }
...
function isNoChoicesError(error) {
    const message = error instanceof Error ? error.message : String(error ?? '');
    return /no choices/i.test(message);
}
function parseOutputs(responseText, outputFormat, outputRelativePath, folderUri) {
    const parsed = extractJson(responseText);
    if (parsed && Array.isArray(parsed.outputs)) {
        return parsed.outputs
            .filter((output) => output.path || output.content)
            .map((output) => {
...
async function writeOutputs(folderUri, outputs) {
    for (const output of outputs) {
        const targetUri = toFolderUri(folderUri, output.uri);
...
function toFolderUri(folderUri, outputUri) {
    if (path.isAbsolute(outputUri.fsPath)) {
        return outputUri;
    }
    return vscode.Uri.joinPath(folderUri, outputUri.fsPath);
}
function getPreviewPanel(outputUri, folderUri, title) {
    ...
    const panel = vscode.window.createWebviewPanel('folderPilot.preview', title, vscode.ViewColumn.Active, { enableScripts: true, localResourceRoots: [folderUri] });
    panel.onDidDispose(() => previewPanels.delete(key));
    previewPanels.set(key, panel);
    return panel;
}
async function openOutput(output, title, folderUri) {
    ...
    if (type === 'md') {
        await vscode.commands.executeCommand('vscode.openWith', output.uri, 'vscode.markdown.preview.editor', vscode.ViewColumn.Active);
        return;
    }
...
async function findNewestOutput(tmpUri) {
    try {
...
function activate(context) {
    ...
    context.subscriptions.push(outputChannel, vscode.window.createTreeView('folderPilotExplorer', { treeDataProvider: provider }), vscode.commands.registerCommand('folderPilot.refresh', () => provider.refresh()), vscode.commands.registerCommand('folderPilot.runSkill', (item) => provider.runSkill(item)), vscode.commands.registerCommand('folderPilot.clearCache', (item) => provider.clearCache(item)));
